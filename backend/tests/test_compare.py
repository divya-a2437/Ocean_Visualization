...
def test_bilinear_interpolate_center_of_uniform_grid():
    ...
    # Midpoint should be the mean of all four corners = 25.0.
...
def test_bilinear_interpolate_handles_nan_corner():
    ...
    # Falls back to mean of valid corners:
    # (10 + 30 + 40) / 3
...
def test_linear_interp_depth_returns_nan_beyond_range():
    depths = [0.0, 100.0]
    values = [30.0, 20.0]

    # Above the shallowest model depth.
    assert math.isnan(
        linear_interp_depth(
            depths,
            values,
